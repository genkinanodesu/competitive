...
ll min_coin(ll x){
  if(x <= 0) return 0;
  return ((x % 5) + (x % 10) / 5 + min_coin(x / 10));
}
...
int main(){
  ...
  ll ans = INF;
  for(ll comb = (1ll << k) - 1; comb < (1ll << n); comb = next_combination(comb)){
    // cout << "comb = " << bitset<10>(comb) << endl;
    ll s = 0;
    REP(i, n){
      if((comb >> i) & 1) s += a[i];
    }
    // printf("s = %lld, min_coin(s) = %lld\n", s, min_coin(s));
    chmin(ans, min_coin(s));
  }
  cout << ans << endl;
}
